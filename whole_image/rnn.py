...
def build_rnn(lr = 0.001, alpha = 0.02, size = 258):
    ...
    rnn_model.add(Dropout(0.2))
    rnn_model.add(Dense(1, activation = 'relu', kernel_regularizer = l2(alpha)))
    opt = Adam(lr = lr)
...
for cv_run in range(1,6):
    for num_cv in range(5):
        ...
        dif12_fit = dif12_rnn.fit([train_seq[:,:,0:640]], train_y,
                                  batch_size = 32, epochs = 100, 
                                  validation_data = ([test_seq[:,:,0:640]], test_y), verbose = 2)
        ...
        hist_csv_file = ''.join(['Code/Info/graphDIF12/cv', str(j), '/run', str(cv_run), '/rnn/adas.csv'])
