...
def build_trans(inputs_shape, ffn_dim, alpha = 0.01, lr = 0.001, rate = 0.1):
    ...
    x = Dropout(rate)(x)
    outputs = Dense(1, activation = 'relu', kernel_regularizer = l2(alpha))(x)
    
    trans_model = Model(inputs = inputs, outputs = outputs)
    opt = Adam(lr = lr)
    trans_model.compile(loss = 'mse', optimizer = opt, metrics = ['mse'])
    
    return trans_model
    
_, _, _, _, _, visits_info = get_data('BP')
#take the extracted nodes for each graph, and build sequences
def trans_seq(num_cv, cv_run, set_type = 'train', start = 0, end = 643):
...
for cv_run in range(1,6):
    for num_cv in range(5):
        ...
        dif12_trans = build_trans(train_seq[0].shape, 256, alpha = 0.0005, lr = 0.00002, rate = 0.1)


            
        dif12_fit = dif12_trans.fit([train_seq], train_y,
                                  batch_size = 32, epochs = 50,  shuffle = True,
                                  validation_data = ([test_seq], test_y), verbose = 2)     
        ...
        hist_csv_file = ''.join(['Code/Info/graphDIF12/cv', str(j), '/run', str(cv_run), '/trans/adas_all.csv'])
