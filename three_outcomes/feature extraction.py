...
la_images, la_diags, la_scores, la_diags_y, la_scores_y, _ = get_data('LA')

ra_images, ra_diags, ra_scores, ra_diags_y, ra_scores_y, _ = get_data('RA')

rt_images, rt_diags, rt_scores, rt_diags_y, rt_scores_y, _ = get_data('RT')
...
for cv_run in range(1, 6):
    ...
    for j in range(5):
        ...
        for gid in train_index:        
            ...
            la_train_images = la_train_images + list(la_images[gid])
            ra_train_images = ra_train_images + list(ra_images[gid])
            rt_train_images = rt_train_images + list(rt_images[gid])
        ...
        for gid in test_index:
            ...
            la_test_images = la_test_images + list(la_images[gid])
            ra_test_images = ra_test_images + list(ra_images[gid])
            rt_test_images = rt_test_images + list(rt_images[gid])
        ...
        la_model = mymodel(la_train_images.shape[1:5], 0.02, 0.0005)
        ra_model = mymodel(ra_train_images.shape[1:5], 0.02, 0.0005)
        rt_model = mymodel(rt_train_images.shape[1:5], 0.02, 0.0005)
        #don't use images whose outcome/score are unavailable
        bp_train = bp_model.fit([np.delete(bp_train_images, nan_index, axis = 0)], seq_diags, batch_size = 40, epochs = 100,
         shuffle = True, validation_data = ([np.delete(bp_test_images, nan_test_index, axis = 0)], seq_test_diags), verbose = 2)
        
        lt_train = lt_model.fit([np.delete(lt_train_images, nan_index, axis = 0)], seq_diags, batch_size = 40, epochs = 100,
         shuffle = True, validation_data = ([np.delete(lt_test_images, nan_test_index, axis = 0)], seq_test_diags), verbose = 2)
        
        la_train = la_model.fit([np.delete(la_train_images, nan_index, axis = 0)], seq_diags, batch_size = 40, epochs = 100,
         shuffle = True, validation_data = ([np.delete(la_test_images, nan_test_index, axis = 0)], seq_test_diags), verbose = 2)
        
        ra_train = ra_model.fit([np.delete(ra_train_images, nan_index, axis = 0)], seq_diags, batch_size = 40, epochs = 100,
         shuffle = True, validation_data = ([np.delete(ra_test_images, nan_test_index, axis = 0)], seq_test_diags), verbose = 2)
        
        rt_train = rt_model.fit([np.delete(rt_train_images, nan_index, axis = 0)], seq_diags, batch_size = 40, epochs = 100,
         shuffle = True, validation_data = ([np.delete(rt_test_images, nan_test_index, axis = 0)], seq_test_diags), verbose = 2)
