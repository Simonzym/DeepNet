...
for cv_run in range(1,6):
    for num_cv in range(5):
        ...
        dif12_rnn = build_rnn(0.0005, 0.05, 640)
        # dif12_img_rnn = build_rnn(0.0005, 0.05, 256)
        metric = []
        img_metirc = []
        for ep in range(1, 101):
            
            dif12_fit = dif12_rnn.fit([train_seq[:,:,0:640]], to_categorical(train_y),
                                      batch_size = 32, epochs = 1, 
                                      validation_data = ([test_seq[:,:,0:640]], to_categorical(test_y)), verbose = 2)
            preds = dif12_rnn.predict(test_seq[:,:,0:640])
        ...
        hist_csv_file = ''.join(['Code/Info/graphDIF12/cv', str(j), '/run', str(cv_run), '/rnn/three_img.csv'])
