...
for cv_run in range(1,6):
    ...
    for num_cv in range(1,6):
        ...
        gnn_model = GCN(640, 256, 128, 128, 0, 3)
        #gnn_noimg_model = GIN(3, 128, 64, 0.1, 3)
        # gnn_img_model = GIN(256, 512, 256, 128, 0, 3)
        
        _, gnn_train_accu, gnn_test_accu, gnn_train_loss, gnn_test_loss, gnn_test_brier, gnn_test_auc = train_model(gnn_model, drange = [0, 640], lr = 0.001, alpha = 0.0001)
        ...
        hist_csv_file = ''.join(['Code/Info/graphDIF12/cv', str(num_cv), '/run', str(cv_run), '/gnn/three_img.csv'])
