...
for cv_run in range(1,6):
    for num_cv in range(1,6):
        ...
        for ep in range(1, 101):
            ...
            dif12_fit = dif12_ffn.fit([train_seq[:,0:640]], to_categorical(train_y),
                                      batch_size = 32, epochs = 1, 
                                      validation_data = ([test_seq[:,0:640]], to_categorical(test_y)))
            preds = dif12_ffn.predict(test_seq[:,0:640])
        ...
        hist_csv_file = ''.join(['Code/Info/graphDIF12/cv', str(num_cv), '/run', str(cv_run), '/ffn/three_img.csv'])
