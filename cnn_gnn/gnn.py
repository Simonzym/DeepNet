...
def train_model(model, epochs = 100, lr = 0.001, drange = [0, 643], alpha = 0.1):
...
for cv_run in range(1,6):
    ...
    for num_cv in range(1,6):
        ...
        gnn_model = GCN(643, 256, 128, 128, 0, 2)

        
        _, gnn_train_accu, gnn_test_accu, gnn_train_loss, gnn_test_loss, gnn_test_brier, gnn_test_auc = train_model(gnn_model, drange = [0, 643], lr = 0.0005, alpha = 0.01)
        ...
        hist_csv_file = ''.join(['Code/Info/graphDIF12/cv', str(num_cv), '/run', str(cv_run), '/gnn/mciad_all.csv'])
