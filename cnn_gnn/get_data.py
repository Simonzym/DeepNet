...
def get_data(folder):
    ...
    for file_id in files_id:
        ...
        for i in range(len(file)):
            ...
            if diag != 1:
                dyn_ncad_diag[gid].append(diag)
                dyn_ncad[gid].append(image_data)
    ...
    for gid in base_gid:
        ...
        dyn_ncad[gid] = np.array(dyn_ncad[gid])
        dyn_ncad_diag[gid] = np.array(dyn_ncad_diag[gid])
    ...
    return dyn_ncad, dyn_ncad_diag, dyn_X, dyn_diag, dyn_score, final_diags, final_scores, dyn_time
